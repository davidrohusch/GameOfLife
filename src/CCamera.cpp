...
Camera::Camera(int _max_x, int _max_y, int _x,
               int _y, int _zoom,
               std::shared_ptr<sf::RenderWindow> _window)
    : max_x(_max_x), max_y(_max_y), x(_x), y(_y), zoom(_zoom), window(_window) {}
    ...
    void Camera::drawGrid() {
      ...
      while (x_pos < window->getSize().x) {
        ...
        vertices.push_back(sf::Vertex(
            sf::Vector2f(position, window->getSize().y), defaultColor));
        x_pos += spacing;
      }
      ...
      while (y_pos < window->getSize().y) {
        ...
        vertices.push_back(sf::Vertex(
            sf::Vector2f(window->getSize().x, position), defaultColor));
        y_pos += spacing;
      }
      // draw it
    ...
    void Camera::drawText(const int &gameSpeed) {
      ...
      std::string format =
          ...
          "  Zoom:" + std::to_string(zoom * 10) + "%" + "   Speed: " + speed;
    ...
    void Camera::move(int _x, int _y, MOVE_METHOD method) {
      if (method == MOVE_METHOD::RELATIVE) {
        x = std::clamp<int>(x + _x, 0, max_x);
        y = std::clamp<int>(y + _y, 0, max_y);
      }
    }
